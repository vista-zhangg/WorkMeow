...
assert.deepStrictEqual(SOURCE_IDS, ['claude', 'codex', 'workbuddy', 'trae', 'opencode', 'zcode']);
...
assert.deepStrictEqual(withValues({ codex: 42 }).map((source) => source.value), [null, 42, null, null, null, null]);
assert.strictEqual(withValues({ opencode: null })[4].value, null);
assert.strictEqual(withValues({ zcode: 7 })[5].value, 7);
assert.deepStrictEqual(withValues(null).map((source) => source.value), [null, null, null, null, null, null]);
