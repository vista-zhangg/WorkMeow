...
这些状态通过 `shared/states.js` 的 `RENDER_STATE_WORDS` 统一清理，保证同一时刻不会把旧状态 class 泄漏到伙伴身上。
...
Claude Code 的 `Stop` 表示本次响应停下，不一定表示整个会话已完成。当载荷仍有 `background_tasks` 或 `session_crons` 时，会话处于「暂停并等待后台工作唤醒」：后端继续保持 `working` 和本轮计时，桌宠只在现有工作动画上显示「后台任务 N 项」或「定时等待 N 项」，不增加额外状态与卡片。

只有后续 `Stop` 中两个集合都清空时，才清理后台标记、停止本轮计时并产生完成徽标/庆祝。`stop_hook_active` 只说明上一次 Stop hook 让 Claude 继续过，不能单独作为本次仍有后台工作的证据。已知后台任务不受 5 分钟忙碌兜底或空闲通知打断；进程结束、中断或新的真实事件仍会正常接管状态。
...
| 响应停下但后台任务/定时唤醒未清空 | 保持 `working`，显示精简数量，不显示完成 |
| 等待权限 | `waiting`，显示授权卡 |
...
视觉槽位、别名和内置 GIF 的唯一来源是 `shared/pet-assets.js`，`renderer/pet.js` 只负责按状态取用。预留状态有词汇和状态机支持，但没有生产事件时不会凭空播放。

### 自定义 GIF 映射

设置页把所有生产视觉状态分成工作、反馈和闲时三组，并显示每组当前参与播放的全部 GIF。情绪别名共享同一视觉槽位：`sorry` → `waiting`、`puzzled` → `needsinput`、`loved` / `excited` → `happy`；`sleeping` 使用 `ambient-sleep`。这样用户只需定制一次，不会出现语义相同但配置分裂的重复入口。

- “新增”将自定义 GIF 追加到现有池；“替换选中”只替换当前选中的默认或自定义 GIF，不影响同状态的其他表情。
- “移出选中”同时适用于默认和自定义 GIF：默认项仅在清单中标记为隐藏，内置文件不会删除；自定义项会清理应用托管的副本，但用户导入的原始文件永不修改或删除。每个状态始终至少保留一个可播放 GIF。
- “恢复默认”清空该状态的自定义池和默认项隐藏记录，完整回到内置资源；旧版“替换整组”清单仍可正常读取和继续编辑。
- 导入由 `backend/gif-normalizer.js` 统一输出 120 × 120 GIF。仅在边框为可安全识别的连通纯色背景时自动透明化；透明或复杂背景保留，避免误伤主体。
- 清单与处理后的副本位于 `~/.agentpaw/pet-assets`，通过只读的 `agentpaw-asset://asset/<uuid>.gif` 协议交给 renderer；协议只解析清单中仍存在的规范 UUID。
- 主进程在修改后同时推送到设置页和桌宠。桌宠清空旧轮换周期并立即重绘当前状态，不需要重启，也不会改变状态机语义。
...
- **loafing 的防误判**：只看「5 秒没事件」会误伤长篇输出——模型可能正在产出、只是还没调工具。`adapter.js` 额外检查 transcript 文件的修改时间，**文件还在长就保持 `working`**，否则会出现「模型疯狂输出、伙伴却躺地上刷手机」。
...
**为什么不直接把语义态换成 `loafing` / `idle`**：`loafing` 现在明确表示「任务进行中的工具间隙」，`idle` 表示「本轮收尾、等你下一句」。若无任务时复用它们，就再也无法一眼分辨「伙伴在摸鱼」到底有没有活在跑，诊断价值归零。
...
- 右键菜单由 `shared/pet-geometry.js` 计算，在工作区安全矩形内选择扇区，按钮避开伙伴的实际可视矩形并保持等距。
- 工具小图标以伙伴的可视矩形为锚点，在左右/上下边缘自动翻转和夹紧，不使用固定的远距离 `bottom/left/right` 偏移。
