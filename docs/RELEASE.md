# WorkMeow Windows 发布手册

这份手册用于发布新的 Windows EXE 安装版和 ZIP 免安装版。目标是只执行一次完整测试和一次本地打包，并让脚本自动完成旧产物清理与校验。
...
3. 运行 `npm test`。
4. 运行 `npm run package:win`。该命令会依次构建、清理旧 `dist`、生成 SHA-256，并执行独立产物校验。
...
不要在本地再次下载 Release 覆盖 `dist`。CI 包和本地包会因为构建时间戳而具有不同哈希，但它们各自的 `latest.yml` 和 `SHA256SUMS.txt` 都是自洽的。回下载只会重复传输约 240 MB 数据，并可能把完整本地文件先截断为下载占位文件。

## 正常产物

完整 Windows 发布后，`dist` 必须只包含以下五个文件：

- `WorkMeow-<version>-Windows-x64.exe`
- `WorkMeow-<version>-Windows-x64.exe.blockmap`
- `WorkMeow-<version>-Windows-x64.zip`
- `latest.yml`
- `SHA256SUMS.txt`
...
- 停在 `building target=zip` 且存在 `7za.exe`：正常。项目当前禁用 ASAR，约 139 MB 的散文件 ZIP 通常需要数分钟，期间可能没有新日志，不要重复启动构建。
