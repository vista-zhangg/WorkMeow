...
const { isKnownAgentId } = require('../shared/agents');
...
function readBody(req, cap, onDone) {
  ...
  req.on('end', () => finish(tooLarge ? null : Buffer.concat(chunks).toString('utf8')));
...
function createServer(deps) {
  ...
  function stateAuthorized(req) {
    return tokenMatches(tokenFromHeaders(req.headers));
  }
  ...
  function handleStatePost(req, res) {
    readBody(req, MAX_STATE_BODY_BYTES, (body) => {
      ...
      if (!data || typeof data !== 'object' || Array.isArray(data)) {
        res.writeHead(400); res.end('object json required'); return;
      }
      ...
      const sid = typeof data.session_id === 'string' && data.session_id.trim()
      ...
      const fields = {
        ...
        // 「等你回复」 can be traced to its source instead of guessed at.
        notificationType: typeof data.notification_type === 'string' && /^[a-z_]{1,32}$/.test(data.notification_type) ? data.notification_type : null,
        ...
        // Trust the agent_id our own hook stamps (validated against the known
        // set). Falls back to 'claude-code' for legacy/Claude hook traffic.
      ...
      try {
        ...
        // A malformed/unsupported event must become an HTTP error rather than
        // tearing down the socket. The latter surfaced in smoke tests as an
        // intermittent ECONNRESET and made the hook caller retry blindly.
        if (!res.headersSent) {
          res.writeHead(500, serverHeaders());
          res.end('state update failed');
        }
      }
    });
  }
  ...
  function handlePermissionPost(req, res) {
    readBody(req, MAX_PERMISSION_BODY_BYTES, (body) => {
      ...
      // AskUserQuestion needs its full structured payload for updatedInput,
      // but the generic permission path only needs bounded display data and a
      // complete identity hash. Reject pathological nesting before retaining
      // a request for up to eight minutes.
      let identityInput;
      try {
        const encoded = JSON.stringify(rawInput);
        if (Buffer.byteLength(encoded, 'utf8') > MAX_PERMISSION_BODY_BYTES) {
          res.writeHead(200, serverHeaders({ 'Content-Type': 'application/json' }));
          res.end(JSON.stringify({ hookSpecificOutput: { hookEventName: 'PermissionRequest', decision: { behavior: 'deny', message: 'tool_input too large' } } }));
          return;
        }
        identityInput = rawInput;
      } catch {
        res.writeHead(200, serverHeaders({ 'Content-Type': 'application/json' }));
        res.end(JSON.stringify({ hookSpecificOutput: { hookEventName: 'PermissionRequest', decision: { behavior: 'deny', message: 'invalid tool_input' } } }));
        return;
      }
      ...
      const sessionId = typeof data.session_id === 'string' && data.session_id.trim()
        ? data.session_id.trim() : null;
      if (!sessionId) {
        // A permission is an authorization boundary.  Never merge malformed
        // requests into a shared "default" session: deny them explicitly so
        // Claude is not left hanging and no other session can answer for them.
        res.writeHead(200, serverHeaders({ 'Content-Type': 'application/json' }));
        res.end(JSON.stringify({
          hookSpecificOutput: {
            hookEventName: 'PermissionRequest',
            decision: { behavior: 'deny', message: 'missing session_id' },
          },
        }));
        return;
      }
      const parsed = {
        ...
        // Full input is used only to compute the request identity in the
        // permission registry; the UI still receives the bounded toolInput.
        identityInput,
  ...
  function start() {
    ...
    server.on('error', (err) => {
      ...
      if (!activePort && err.code === 'EADDRINUSE') return;
    });
